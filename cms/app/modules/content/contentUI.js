...
define(['json!modules/content/lang/' + lang.key + '.json', 'central'], function (mLang, central) {
                        ...
                        },
                        {
                            ...
                            children: [
                                ...
                                },
                                {
                                    ...
                                    on: {
                                        click: function () {
                                            central.get('selectedNode').publish().then(function () {
                                                contentUI.btnPublish.removeClass('tme-highlight tme-highlight-ready', false).addClassTemporary('tme-success', 2000, false);
                                            }).done();
                                        }
                                    }
                                },
            ...
            for (var key in struct) {
                ...
                (function (key, events, definition) {
                    ...
                    changeListener.listenTo(events, 'change', _.debounce(function (e) {
                        central
                            .get('selectedNode')
                            .updateContent(key + (definition.repeat ? '[' + e.index + ']' : '') + (e.key ? '.' + e.key : ''), e.value)
                            .then(updatePublishStatus);
                    }, 10));
                    changeListener.listenTo(events, 'remove', function (e) {
                        central
                            .get('selectedNode')
                            .updateContent(key + '[' + e.index + ']' + (e.key ? '.' + e.key : ''), undefined)
                            .then(updatePublishStatus);
                        ;
                    });
    ...
    function updatePublishStatus(n) {
        n = central.get('selectedNode');
        if (n.get('needsPublish')) {
            contentUI.btnPublish.el.addClass('tme-highlight tme-highlight-ready');
        } else {
            contentUI.btnPublish.el.removeClass('tme-highlight tme-highlight-ready');
        }
    }
    ...
    function updateSelectedNode() {
        ...
        changeListener.stopListening();
        changeListener.listenTo(central.get('selectedNode'), 'change', function () {
            updatePublishStatus();
        });
        updatePublishStatus();
